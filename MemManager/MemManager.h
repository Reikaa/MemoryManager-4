...
public:
	...
	//WRITE TO MEMORY
	int Write(int adress, char ch);
	int wr(int adress, std::string data);


	//Pobiera rozkaz z danego adresu
	std::string Get(int LR);
};
...
public:
	struct PCB
	{
		...
		int LR;
		std::string name;
		std::vector<MemoryManager::PageTableData> *page_table;

		PCB(int PID, int LR, std::string name, std::vector<MemoryManager::PageTableData> *page_table) {
			this->PID = PID;
			this->LR = LR;
			this->name = name;
			this->page_table = page_table;
	...
	void showRunning();
	void selectRunning(int PID);
	void GO(MemoryManager &mm);
	void removeProcess(int PID, MemoryManager &mm);
	bool checkPID(int n);
