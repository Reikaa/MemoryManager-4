...
int MemoryManager::Write(PCB *process, int adress, char ch) {
	int prev = adress - 1;
	int next = adress + 1;
	int stronica = adress / 16;
	std::string str;

	if (adress > process->page_table->size() * 16 - 1 || adress < 0) {
	...
	if (process->->page_table->at(stronica).bit == 0)
		LoadtoMemory(PageFile[running->PID][stronica], stronica, process->PID, process->page_table);

	str = PageFile[process->PID].at(stronica).data;
	...
	if (adress != 0) {
		...
		if (prev / 16 != stronica) {
			std::string pom = PageFile[process->PID].at(stronica - 1).data;
			if (pom[15] != ' ') {
				std::cout << "Nie wpisano znaku do pamieci podano zajety adres" << std::endl;
				return -1;
			}
	...
	//sprawdzam nastepny
	if (adress != process->page_table->size() * 16 - 1) {
		if (next / 16 != stronica) {
			std::string pom = PageFile[process->PID].at(stronica + 1).data;
			if (pom[0] != ' ') {
				std::cout << "Nie wpisano znaku do pamieci podano zajety adres" << std::endl;
	...
	RAM[process->page_table->at(stronica).frame * 16 + adress - (16 * stronica)] = ch;
